...
// PushCommand an approuter service instance
...
func (c *ApprouterPushCommand) Execute(args []string) ExecutionStatus {
	...
	// Parse arguments
	flagSet := flag.NewFlagSet("html5-approuter-push", flag.ContinueOnError)
	fileFlag := flagSet.String("file", "", "Path to manifest.yaml file")
	fileFlagAlias := flagSet.String("f", "", "Path to manifest.yaml file")
	flagSet.Parse(args)

	// Normalize arguments and aliases
	file := *fileFlag || *fileFlagAlias
	log.Tracef("File flag: %v\n", file)
	...
	if len(args) == 0 {
		...
		return c.PushApprouter(dirs, "", file)
	}
...
func (c *PushApprouterCommand) PushApprouter(appPaths []string, appHostGUID string, redeploy bool) ExecutionStatus {
	var err error
	var zipFiles []string
