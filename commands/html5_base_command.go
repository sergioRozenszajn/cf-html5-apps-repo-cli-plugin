...
func (c *HTML5Command) GetHTML5Context(context Context) (HTML5Context, error) {
	...
	ui.Say("Using HTML5 Repo Service: '%s'", serviceName)
	...
	// Find app-router service plan
	log.Tracef("Looking for app-router service plan\n")
	var appRouterServicePlan *models.CFServicePlan
	for _, plan := range servicePlans {
		if plan.Name == "app-router-dev" {
			appRouterServicePlan = &plan
			break
		}
	}
	if appRouterServicePlan != nil {
		html5Context.HTML5AppRouterServicePlan = appRouterServicePlan
	}
	...
	// Get list of service instances of app-router plan
	log.Tracef("Getting service instances of '%s' service app-router plan (%+v)\n", serviceName, appRouterServicePlan)
	var appRouterServiceInstances []models.CFServiceInstance
	appRouterServiceInstances, err = clients.GetServiceInstances(c.CliConnection, context.SpaceID, []models.CFServicePlan{*appRouterServicePlan})
	if err != nil {
		return html5Context, errors.New("Could not get service instances for app-runtime plan: " + err.Error())
	}
	html5Context.HTML5AppRouterServiceInstances = appRouterServiceInstances

	// Create instance of app-runtime plan if needed
	if appRuntimeServicePlan != nil && appRouterServicePlan == nil {
...
type HTML5Context struct {
	...
	// Pointer to html5-apps-repo app-router service plan
	HTML5AppRouterServicePlan *models.CFServicePlan
	// Service instances of html5-apps-repo app-runtime service plan
	HTML5AppRuntimeServiceInstances []models.CFServiceInstance
	// Service instances of html5-apps-repo app-router service plan
	HTML5AppRouterServiceInstances []models.CFServiceInstance
	// Pointer to html5-apps-repo app-runtime service instance
	HTML5AppRuntimeServiceInstance *models.CFServiceInstance
	// Pointer to html5-apps-repo app-router service instance
	HTML5AppRouterServiceInstance *models.CFServiceInstance
